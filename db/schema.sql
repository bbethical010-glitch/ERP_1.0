...
ALTER TABLE businesses ADD COLUMN IF NOT EXISTS allow_negative_stock BOOLEAN NOT NULL DEFAULT FALSE;
...
ALTER TABLE financial_years ADD COLUMN IF NOT EXISTS is_closed BOOLEAN NOT NULL DEFAULT FALSE;
ALTER TABLE financial_years ADD COLUMN IF NOT EXISTS closed_at TIMESTAMPTZ;
ALTER TABLE financial_years ADD COLUMN IF NOT EXISTS closed_by TEXT;
...
ALTER TABLE inventory_transactions ADD COLUMN IF NOT EXISTS posting_date DATE;
UPDATE inventory_transactions SET posting_date = transaction_date WHERE posting_date IS NULL;
ALTER TABLE inventory_transactions ALTER COLUMN posting_date SET NOT NULL;

CREATE INDEX IF NOT EXISTS idx_inventory_tx_business_product ON inventory_transactions (business_id, product_id);
CREATE INDEX IF NOT EXISTS idx_inventory_tx_business_product_date
  ON inventory_transactions (business_id, product_id, posting_date);
...
CREATE INDEX IF NOT EXISTS idx_purchase_voucher_business_bill_date
  ON purchase_voucher (business_id, bill_date);
...
CREATE INDEX IF NOT EXISTS idx_purchase_lines_business_product
  ON purchase_lines (business_id, product_id);
CREATE INDEX IF NOT EXISTS idx_purchase_lines_business_asset
  ON purchase_lines (business_id, asset_account_id);
...
DROP TRIGGER IF EXISTS trg_validate_ledger_postings_origin ON ledger_postings;
